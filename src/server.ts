...
server.statefulSession(async(io, { user, sessionId }) => {
  ...
  while (true) {
    ...
    if (componentChoice === 'paragraph') {
      const { destroy } = await io.display.paragraph('Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.')
      await new Promise(r => setTimeout(r, 5000))
      destroy()
    }
    if (componentChoice === 'heading') {
      const { destroy } = await io.display.heading('This is a heading!')
      await new Promise(r => setTimeout(r, 5000))
      destroy()
    }
